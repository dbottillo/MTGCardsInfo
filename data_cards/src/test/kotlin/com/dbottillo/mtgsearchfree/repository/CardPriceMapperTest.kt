...
class CardPriceMapperTest {
    ...
    fun `should return result error if api tcg price is not successful`() {
        ...
        val result = underTest.map(api)

        assertThat(result).isEqualTo(TCGPriceResult.Error)
    }
    ...
    fun `should return result price if it's the only one in the results`() {
        ...
        assertThat(result).isEqualTo(TCGPriceResult.Price(TCGPrice(hiPrice = "30.0", avgPrice = "40.0", lowprice = "23.0")))
    }
    ...
    fun `should return normal price if there is more than one`() {
        ...
        assertThat(result).isEqualTo(TCGPriceResult.Price(TCGPrice(hiPrice = "130.0", avgPrice = "140.0", lowprice = "123.0")))
    }
}